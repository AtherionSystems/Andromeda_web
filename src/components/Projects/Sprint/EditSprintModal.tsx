...
import { updateSprint } from "../../../api/projects";
import type { ApiProject, ApiSprint, SprintStatus } from "../../../types/api";
...
function EditSprintModal({ isOpen, project, sprint, onClose, onSaved }: Props) {
  ...
  const [startDate, setStartDate] = useState<Date | undefined>(parseDate(sprint.startDate));
  const [endDate, setEndDate] = useState<Date | undefined>(parseDate(sprint.dueDate));
  const [activeCalendar, setActiveCalendar] = useState<ActiveCalendar>(null);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Re-sync when the sprint prop changes (e.g. different sprint opened)
  useEffect(() => {
    if (!isOpen) return;
    setName(sprint.name);
    setGoal(sprint.goal ?? "");
    setStatus(sprint.status);
    setStartDate(parseDate(sprint.startDate));
    setEndDate(parseDate(sprint.dueDate));
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const updated = await updateSprint(project.id, sprint.id, {
        name: name.trim(),
        goal: goal.trim() || null,
        status,
  ...
  const dateButtonClass = (active: boolean) =>
    `w-full rounded border px-3 py-2.5 text-sm text-left transition-colors focus:outline-none focus:ring-2 focus:ring-[#C74634]/30 focus:border-[#C74634] ${
      active ? "border-[#C74634] ring-2 ring-[#C74634]/30" : ""
    } ${darkMode ? "bg-slate-800 border-slate-700 text-slate-100" : "bg-[#f5f5f5] border-transparent text-slate-700"}`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[1px] px-4">
      ...
      >
        <button
          ...
          onClick={onClose}
          aria-label="Close"
          className={`absolute top-4 right-5 leading-none z-10 transition-colors ${
        ...
        <div className="flex">
          ...
          <div className="flex-1 p-8 min-w-0">
            <h2 id="edit-sprint-title" className={`text-2xl font-semibold mb-1 ${darkMode ? "text-slate-100" : "text-slate-800"}`}>
              Edit Sprint
            </h2>
            <p className={`text-[10px] tracking-[1.2px] uppercase font-semibold mb-5 ${darkMode ? "text-slate-500" : "text-slate-400"}`}>
              {project.name}
            </p>
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                <input ref={nameInputRef} type="text" value={name} onChange={(e) => setName(e.target.value)} maxLength={255} className={inputClass} />
              </div>

              <div className="mb-4">
                <label className={labelClass}>Sprint Goal</label>
                <textarea value={goal} onChange={(e) => setGoal(e.target.value)} rows={3} className={`${inputClass} resize-none`} />
              </div>

              <div className="mb-4">
                <label className={labelClass}>Status</label>
                <select value={status} onChange={(e) => setStatus(e.target.value as SprintStatus)} className={inputClass}>
              ...
              <div className="mb-6 grid grid-cols-2 gap-3">
                <div>
                  ...
                  <button type="button" onClick={() => setActiveCalendar((p) => (p === "start" ? null : "start"))} className={dateButtonClass(activeCalendar === "start")}>
                    <span className={startDate ? (darkMode ? "text-slate-100" : "text-slate-800") : (darkMode ? "text-slate-500" : "text-slate-400")}>
                      {fmtDate(startDate)}
                    </span>
                  </button>
                </div>
                <div>
                  ...
                  <button type="button" onClick={() => setActiveCalendar((p) => (p === "end" ? null : "end"))} className={dateButtonClass(activeCalendar === "end")}>
                    <span className={endDate ? (darkMode ? "text-slate-100" : "text-slate-800") : (darkMode ? "text-slate-500" : "text-slate-400")}>
                      {fmtDate(endDate)}
                    </span>
                  </button>
                </div>
          ...
          <div className="flex flex-col p-6 justify-start min-w-[260px]">
            {activeCalendar === null ? (
              <div className="flex flex-col items-center justify-center h-full gap-3">
                <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" className={darkMode ? "text-slate-700" : "text-slate-300"}>
                  <rect x="3" y="4" width="18" height="18" rx="2" />
                  <line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" />
                </svg>
                <p className={`text-[11px] text-center ${darkMode ? "text-slate-600" : "text-slate-400"}`}>
                  Click a date field<br />to open the calendar
                </p>
              </div>
            ) : (
              <>
                <p className={`text-[10px] tracking-[1.2px] uppercase font-semibold mb-3 ${darkMode ? "text-slate-400" : "text-slate-500"}`}>
                ...
                <Calendar
                  ...
                  onSelect={(d) => {
                    if (activeCalendar === "start") { setStartDate(d); setActiveCalendar("end"); }
                    else { setEndDate(d); setActiveCalendar(null); }
                  }}
                  disabled={activeCalendar === "end" && startDate ? { before: startDate } : undefined}
                  captionLayout="label"
                />
                <button
                  type="button"
                  onClick={() => { if (activeCalendar === "start") setStartDate(undefined); else setEndDate(undefined); }}
