...
const BacklogColumn: React.FC<ColumnProps> = ({
  ...
  return (
    <div
      className={`flex h-full min-h-0 w-full max-w-[360px] flex-col rounded-xl p-5 overflow-hidden ${darkMode ? "bg-gradient-to-b from-slate-700 to-slate-800 text-slate-100" : "bg-gradient-to-b from-[#e4eef2] to-[#F4F7F9] text-slate-800"}`}
    >
      <div className="mb-6 px-1">
        <h3 className="font-serif text-2xl">{title}</h3>
        <div className="mt-2 flex flex-col items-start gap-2">
          <span
            className={`text-[10px] font-bold tracking-[0.15em] uppercase ${darkMode ? "text-red-400" : "text-red-700"}`}
          >
            {subtitle}
          </span>
        </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto pr-1">
        ...
        {tasks.length === 0 && (
          <div
            className={`flex flex-col items-center justify-center rounded-lg border-2 border-dashed py-12 ${darkMode ? "border-slate-600" : "border-slate-200"}`}
          >
            <p
              className={`text-[11px] font-medium uppercase tracking-widest ${darkMode ? "text-slate-300" : "text-slate-400"}`}
            >
              No Tasks
            </p>
          </div>
      ...
      <div
        className={`mt-4 flex items-center justify-center gap-1 rounded-full px-1 py-1 shadow-sm flex-shrink-0 ${darkMode ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"}`}
      >
        <button
          type="button"
          onClick={() => {
        ...
        <span
          className={`min-w-10 text-center text-[10px] font-semibold ${darkMode ? "text-slate-300" : "text-slate-500"}`}
        >
